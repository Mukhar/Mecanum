...
void loop()
{
  ...
  if (positionValF<69)
    {
      if(positionValF<69 && positionValF >40)     //left from line    
      {
        storF=1;
      }
      if(positionValF >= 0 && positionValF <40)     //right from line
      {storF=0;
      }
    }
...
if (positionValB<69)
    {
      if(positionValB<69 && positionValB >40)     //left from line    
      {
        storB=1;
      }
      if(positionValB >= 0 && positionValB <40)     //right from line
      {storB=0;
      }
    }

    if(positionValF >70 && positionValB<70)       // F is out B is in
      {
        if(storF == 1) { 
                 Bcw();                       //Rotate clockwise about B
          }
        if(storF == 0) {
                  Bacw();                      //Rotate anticlockwise about B
          }
        }

     if(positionValB >70 && positionValF<70)       // B is out F is in
      {
        if(storB == 1) { 
                 Facw();                       //Rotate anticlockwise about F
          }
        if(storB == 0) {
                    Fcw();                    //Rotate clockwise about F
          }
        }

      if(positionValB >70 && positionValF>70)      // both are out
      {
        if (storB>70 && storF>70)
        {
          if(storB == 1 && storF==1) { 
                     Lft();               //move left
          }
